...
const fetchEmailTemplates = async (query = '', page = 1) => {
  try {
    const response = await fetch(`${process.env.SERVER_API}/email-template?q=${query}&page=${page}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${token}`,
        'Content-Type': 'application/json',
  ...
  const handleSubmit = async (templateId) => {
    try {
      ...
      const responseCampaign = await fetch(`${process.env.SERVER_API}/campaigns/${id}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (responseCampaign.ok) {
        ...
        if (campaignData.data.email_template_id) {
          ...
          const updateResponse = await fetch(`${process.env.SERVER_API}/campaigns/${id}`, {
            method: 'PUT',
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json',
          ...
          const createResponse = await fetch(`${process.env.SERVER_API}/campaigns/${id}`, {
            method: 'PUT',
            headers: {
              Authorization: `Bearer ${token}`,
              'Content-Type': 'application/json',
